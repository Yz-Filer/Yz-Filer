...
- テキストエディタの指定などの初期設定は、F8キーで設定できます。  
  設定項目は以下の通りです。
    ...
    - Toolタブ
      ...
      - テキストビューアで読み込む最大容量  
        処理速度がかかる（文字コード判定）ため、容量を制限してます。  
        変更時は、MB単位のため注意して下さい。
     
      - SFTP/SCPでのバイナリビューアで読み込む最大容量  
        転送速度がかかるため、容量を制限してます。  
        変更時は、MB単位のため注意して下さい。
    ...
    - SSHタブ  
      ...
      - SSHクライアントログインマクロ  
        ...
        追加コマンド（%SSH_ADD_MACRO%）は、以降の2つの項目どちらかが自動的に選択されます
    ...
    - Colorタブ  
      ...
      自分で設定した色は次回起動時にも反映されますがバックアップされません  
...
## おわりに
自分で使う用に作成したのですが、某掲示板できいたところ使いたいという方がおられましたので公開しました。  
その後、SCP(SSH)対応が思いのほか便利だったので、バージョンアップを行っています。
